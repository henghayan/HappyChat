...
def apply_delta(base_model_path, target_model_path, delta_path, torch_dtype=torch.bfloat16, low_cpu_mem_usage=True):
    print(f"Loading the base model from {base_model_path}")
    base = AutoModelForCausalLM.from_pretrained(
        base_model_path, torch_dtype=torch_dtype, low_cpu_mem_usage=low_cpu_mem_usage)
    base_tokenizer = AutoTokenizer.from_pretrained(
        base_model_path, use_fast=False)

    print(f"Loading the delta from {delta_path}")
    delta = AutoModelForCausalLM.from_pretrained(delta_path, torch_dtype=torch_dtype,
